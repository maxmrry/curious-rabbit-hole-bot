...
def build_feed(selected_items, daily_principle):
    ...
    # Entry 1: The AI Daily Reminder (Power of Bad logic)
    daily_reminder = get_daily_reminder() # Make sure to import this at the top!
    fe_rem = fg.add_entry()
    ...
    fe_rem.description(f"<b>Active Rewire:</b> {daily_reminder}")
    fe_rem.pubDate(now)
    fe_rem.id(f"protocol-{now.strftime('%Y%m%d')}")

    # Entry 2: The Daily Anchor (Timeless Adage)
    ...
    for item in selected_items:
        ...
        # Inject the thumbnail image into the description text like your old project
        final_desc = f"<b>Source:</b> {item['source_name']}<br><br>"
        if item.get('image_url'):
            fe.enclosure(item['image_url'], 0, 'image/jpeg') 
            final_desc += f"<img src='{item['image_url']}' alt='thumbnail'/><br><br>"
